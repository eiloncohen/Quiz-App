...
class UI(Tk):
    def __init__(self):
        ...
        self.known_button = Button(image=self.wrong_img, highlightthickness=0)
        self.known_button.grid(row=2, column=1)
        self.mainloop()

    # def enter_question_to_canvas(self, question):
    #     self.canvas.itemconfig(self.canvas, text=question['question'], fill=THEME_COLOR)
